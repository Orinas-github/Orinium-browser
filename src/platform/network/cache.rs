...
impl Cache {
    ...
    pub async fn set(&self, url: &Url, body: Vec<u8>, headers: Vec<(String, String)>) {
        ...
        if let Some((_, cc)) = headers.iter().find(|(n, _)| n.to_lowercase() == "cache-control") {
            if let Some(pos) = cc.find("max-age=") {
                if let Ok(max_age) = cc[pos+8..].split(|c: char| !c.is_ascii_digit()).next().unwrap_or("0").parse::<u64>() {
